function calc_chialphaalpha(; op_alpha::Matrix{ComplexF64},
    ...
    t_avg_alpha::Float64 = thermal_average(Ep=Ep, Vp=Vp, operator=op_alpha, T=T, mode=abs)
...
function cef_susceptibility_powder!(single_ion::mag_ion, bfactors::DataFrame,
    ...
    @eachrow! calc_grid begin
        @newcol :CHI_CALC::Vector{Float64}
        chiperp = calc_chi(single_ion.g, spin_ops=spin_ops.*[1.0, 0.0, 0.0], Ep=E, Vp=V, T=:T)
        chipara = calc_chi(single_ion.g, spin_ops=spin_ops.*[0.0, 0.0, 1.0], Ep=E, Vp=V, T=:T)
